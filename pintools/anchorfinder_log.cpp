...
void getctx(ADDRINT addr, ADDRINT inssize, CONTEXT *fromctx, ADDRINT raddr, ADDRINT waddr)
{
    ...
    if (iter != opcmap.end())
        opcmap.erase(iter);
    cout << hex << addr << dec << " : " << opcmap[addr] << endl;
...
int main(int argc, char *argv[])
{
    ...
    char *c_file = new char[KnobTestProgram.Value().size()];
    memcpy(c_file, buf, i);
    c_file[i] = '.';
    c_file[i + 1] = 'c';
    c_file[i + 2] = '\0';
    ...
    PIN_AddContextChangeFunction(OnsigDebug, 0);
